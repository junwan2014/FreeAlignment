...
from detectron2.data.datasets.coco import load_coco_json, load_sem_seg_wflw
...
def register_wflw(root, max_num):
    root = root+'WFLW/'
    meta = _get_voc_meta(CLASS_NAMES)
    image_dir = os.path.join(root, 'WFLW_images/')
    gt_dir = os.path.join(root, 'WFLW_images/')
    train_json = root + '/face_landmarks_wflw_train.csv' #'/face_landmarks_wflw_train.csv'
    val_json = root + '/face_landmarks_wflw_test.csv'
    all_name = 'WFLW_train'
    DatasetCatalog.register(all_name, lambda: load_sem_seg_wflw(image_dir, train_json, max_num, 'train')) #D:\python_work\lib\detectron_repo\detectron2\data\datasets\coco.py
    ...
    gt_dir = os.path.join(root, 'WFLW_images/')
    all_name = 'WFLW_val'
    panoptic_root = 'E:/datasets/UFA/annotations/WFLW_val/'
    panoptic_json = 'E:/datasets/UFA/annotations/WFLW_gt_ibug.json'
    DatasetCatalog.register(all_name,lambda: load_sem_seg_wflw(image_dir, val_json, max_num, 'test'),)
