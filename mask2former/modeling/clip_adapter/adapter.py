...
class ClipAdapter(nn.Module):
    ...
    def forward(self, image: torch.Tensor, text: List[str], task_name: str, **kwargs):
        image = self._preprocess_image(image, **kwargs) #预处理，啥都没做
        text_feature = self.get_text_features(text, task_name)  # k,feat_dim [156, 512]
        image_features = self.get_image_features(image)#[32, 512]使用clip对图像提取特征
    ...
    def _get_text_features(self, noun_list: List[str], task_name: str):
        if not self.prompt_learner.with_trainable_params:
            ...
            if len(left_noun_list) > 0: #应该是已知类的处理 left_text_features [156, 512] 与 已知的156类对应
                left_text_features = self.prompt_learner( left_noun_list, self.clip_model, task_name) # mask2former/modeling/clip_adapter/text_prompt.py line 121
            ...
            text_features = self.prompt_learner(noun_list, self.clip_model, task_name) # mask2former/modeling/clip_adapter/text_prompt.py line 117
            self.text_feature_buffer.update({noun: text_feature.detach() for noun, text_feature in zip(noun_list, text_features)}) #得到一个字典
    ...
    def get_image_features(self, image: torch.Tensor):
        image_features = self.clip_model.visual(image)# 32x512
...
class MaskFormerClipAdapter(ClipAdapter):
    ...
    def forward(
        ...
        image, valid_flag = self._preprocess_image(image, mask, normalize=normalize) #取有用的mask对应的图像，valid_flag返回所有的查询是否有用
        if image is None:
            return None, valid_flag
        if isinstance(image, list):
            image_features = torch.cat([self.get_image_features(image_i) for image_i in image], dim=0)
        else:
            image_features = self.get_image_features(image) #取有用的查询对应的图像特征
        text_feature = self.get_text_features(text, task_name)  # k,feat_dim，取对应的文本特征
        return self.get_sim_logits(text_feature, image_features), valid_flag

    def _preprocess_image(self, image: torch.Tensor, mask: torch.Tensor, normalize: bool = True):
        ...
        """
        dtype = mask.dtype
        bin_mask = mask > self.mask_thr # >0.5设置为True,其余为False
        valid = bin_mask.sum(dim=(-1, -2)) > 0 #统计有效的mask, 根据最后两个维度统计true的数量，大于0表示mask有用，否则没有勇
        bin_mask = bin_mask[valid] #取有用的mask, [ valid_num, w, h]
        mask = mask[valid]
        if not self.mask_matting:
            mask = bin_mask
        bin_mask = BitMasks(bin_mask) # [ valid_num, w, h]
        bboxes = bin_mask.get_bounding_boxes() # [ valid_num, 4]
        # 根据预测的box截图图片，然后使用mask进行遮罩，得到预测的遮罩图像
        regions = [crop_with_mask( image.type(dtype), single_mask.type(dtype), bbox, fill=self.mask_fill, expand_ratio=self.mask_expand_ratio, )[None, ...]for bbox, single_mask in zip(bboxes, mask)]
        if len(regions) == 0:
            return None, valid
        if normalize:
            regions = [(r - self.pixel_mean) / self.pixel_std for r in regions]
        # resize 把遮罩图像同意resize为224x224大小，方便输入clip模型
        if self.region_resized:
            regions = [F.interpolate(r, size=(224, 224), mode="bicubic", align_corners=True) for r in regions]
            regions = torch.cat(regions)
        return regions, valid

    def get_text_features(self, noun_list: List[str], task_name: str):
        object_text_features = self._get_text_features(noun_list, task_name) #针对已经的类提取对应的文本特征
        non_object_text_features = ( self.non_object_embedding / self.non_object_embedding.norm(dim=-1, keepdim=True)) #非类？不知道啥意思
