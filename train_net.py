...
from mask2former.data.datasets.register_300w import register_300w
from mask2former.data.datasets.register_aflw import register_aflw
from mask2former.data.datasets.register_cofw import register_cofw
from mask2former.data.datasets.register_wflw import register_wflw
...
from mask2former.data import (
    ...
    MaskFormer300WMapper,
    MaskFormerWFLWMapper,
    MaskFormerAFLWMapper,
    MaskFormerCOFWMapper,
...
from mask2former.data import (
    ...
    dataset_sample_per_task_class_wan,
)
...
class Trainer(DefaultTrainer):
    ...
    def build_evaluator(cls, cfg, dataset_name, output_folder=None):
        ...
        if evaluator_type in ["sem_seg", "ade20k_panoptic_seg"]:
            if cfg.PSEUDO:
                ...
                evaluator = GeneralizedPanopticEvaluator(dataset_name, output_folder)
    ...
    def build_train_loader(cls, cfg):
        ...
        # Instance segmentation dataset mapper
            ...
            return build_detection_train_loader(cfg, mapper=mapper)
        elif cfg.INPUT.DATASET_MAPPER_NAME == "mask_former_alignment":
            mapper_300w = MaskFormer300WMapper(cfg, True, cfg.SOLVER.MODE) #这里配置数据集
            mapper_wflw = MaskFormerWFLWMapper(cfg, True, cfg.SOLVER.MODE)
            mapper_cofw = MaskFormerCOFWMapper(cfg, True, cfg.SOLVER.MODE)
            mapper_aflw = MaskFormerAFLWMapper(cfg, True, cfg.SOLVER.MODE)
            mapper=[]
            mapper.append(mapper_300w)
            mapper.append(mapper_wflw)
            mapper.append(mapper_cofw)
            mapper.append(mapper_aflw)#多个数据集合并
            dataset = dataset_sample_per_task_class_wan(cfg)
    ...
    def build_test_loader(cls, cfg, dataset_name):
        ...
        if cfg.ORACLE:
            ...
            mapper = None
        if dataset_name=='300W_val':
            mapper = MaskFormer300WMapper(cfg, False)
        elif dataset_name=='WFLW_val':
            mapper = MaskFormerWFLWMapper(cfg, False)
        elif dataset_name == 'AFLW_val':
            mapper = MaskFormerAFLWMapper(cfg, False)
        else:
            mapper = MaskFormerCOFWMapper(cfg, False)
...
def setup(args):
    ...
    """
    cfg = get_cfg() # D:\python_work\lib\detectron_repo\detectron2\config\defaults.py line 22
...
def main(args):
    ...
    _root = 'E:/datasets/'
    register_aflw(_root, cfg.DATASETS.MAX_IMAGE_SIZE)
    register_300w(_root, cfg.DATASETS.MAX_IMAGE_SIZE)
    register_wflw(_root, cfg.DATASETS.MAX_IMAGE_SIZE)
    register_cofw(_root, cfg.DATASETS.MAX_IMAGE_SIZE)
    ...
    return trainer.train() # D:\python_work\lib\detectron_repo\detectron2\engine\train_loop.py   line 144


if __name__ == "__main__":
    args = default_argument_parser().parse_args() # D:\python_work\lib\detectron_repo\detectron2\engine\defaults.py line 110
