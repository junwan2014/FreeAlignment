...
class MaskFormerInteractionHead(nn.Module):
    ...
    def layers2(self, features, text_feat=None, mask=None):
        #MSDeformAttnPixelDecoder 模块的编码器, 用于预测mask, mask_features[bs, 256, 112, 112], transformer_encoder_features[bs, 256, 14, 14], multi_scale_features:[bs, 256, 14, 14], [bs, 256, 28, 28] [bs, 256, 56, 56]
        #mask_features由res_feature和查询更新之后的特征+masked卷积操作得到，multi_scale_features查询得到的多尺度特征，transformer_encoder_features是multi_scale_features查询得到的多尺度特征尺寸最小的特征
        mask_features, transformer_encoder_features, multi_scale_features = self.pixel_decoder.forward_features(features)
        #Semantic Context Interaction 模块的解码器， 利用多尺度的视觉特征查询文本特征对视觉特征进行更新  mask2former/modeling/heads/module.py line 159
        ...
        if self.transformer_in_feature == "multi_scale_pixel_decoder":
            predictions = self.predictor(multi_scale_features, mask_features, mask) #mask2former/modeling/transformer/mask2former_transformer_predictor.py line 367
        else:
