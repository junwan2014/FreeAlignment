...
def crop(img, center, scale, output_size, image_name, rot=0, ):
    ...
    if old_x[0]-old_x[1]>=0 or old_y[0]-old_y[1]>=0:
        print(image_name)
...
def show_preds1(image, preds):
    # plt.figure()
    plt.imshow(image)
    for pred in preds:
        plt.scatter(int(pred[0]), int(pred[1]), s=10, marker='.', c='r')
...
def show_preds(image, preds):
    ...
    for pred in preds:
        num = num + 1
        image = cv2.putText(image, str(num), (int(pred[0]+5), int(pred[1]+5)), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 1)
        image = cv2.circle(image, (int(pred[0]), int(pred[1])), 1, (255, 0, 0), 2)
    plt.imshow(image)
    plt.show()