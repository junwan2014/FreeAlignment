...
class ContextInteraction(nn.Module):
    ...
    def forward(self, mask_features, transformer_encoder_features, multi_scale_features, text_feat, attention_mask):
        ...
        q0, q1, q2 = multi_scale_features[0], multi_scale_features[1], multi_scale_features[2] #多尺度特征
        q3 = mask_features #遮罩的特征
        k = text_feat
        v = text_feat

        output = []
        bs, _, h, w = q0.shape
        k = k.expand(bs, k.shape[0], k.shape[1]) # k,v 都是文本特征， [bs, 157, 512]
        v = v.expand(bs, v.shape[0], v.shape[1])
        for q_index, q in enumerate([q0, q1, q2, q3]): #多尺度的视觉特征和遮罩之后的视觉特征分别查询分本特征
            bs, _, h, w = q.shape
            q = q.flatten(2).transpose(1, 2) #1x400x256, 1x1600x256, 1x6400x256, 1x25600x256
            q = self.layer_norm_q_1(q) #[bs, 196, 256]
            k, v = self.layer_norm_k_1(k), self.layer_norm_k_1(v) #[bs, 9, 512]
            delta_q = self.attn(q, k, v, attention_mask=attention_mask)[0] #1x400x256
            if self.use_layer_scale:
                q = q + self.drop_path(self.gamma * delta_q)
            else:
                q = q + delta_q
            q = q.transpose(1, 2).contiguous().view(bs, -1, h, w) #1x256x20x20, 1x256x40x40, 1x256x80x80, 1x256x160x160
            output.append(q)
