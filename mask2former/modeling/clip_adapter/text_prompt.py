...
class LearnablePromptExtractor(PromptExtractor):
    def __init__(self, prompt_dim: int, prompt_shape: Tuple[int, int], task_prompt_shape: int, task_names: List[str]):
        ...
        if len(task_names) > 0:
            self.task_prompt = self._init_task_prompt(task_prompt_shape) # length of task prompt, 默认8个类别
        else:
    ...
    def forward(self, noun_list: List[str], clip_model: nn.Module, task_name: str):
        ...
        if task_name is not None:
            task_embed = self.task_embeddings[task_name].to(self.prefix_prompt.device) #取对应的任务的embedding [5, 512]
            task_prompt = [task_embed]
            if self.task_prompt is not None:
                task_prompt.append(self.task_prompt) #[8, 512]
            task_prompt = torch.cat(task_prompt) #[13, 512]
        else:
            task_prompt = torch.Tensor([]).to(self.start_signal.device)
            self.task_prompt = None
        
        prefix = [self.start_signal] #[1, 512]
        if self.prefix_prompt is not None: #[16, 512]
            prefix.append(self.prefix_prompt)
        prefix = torch.cat(prefix)#[17, 512]
        suffix = [self.dot_signal, self.end_signal] #both [1, 512]
        if self.suffix_prompt is not None:
            suffix.insert(0, self.suffix_prompt)
        suffix = torch.cat(suffix)#[2, 512]
        # only process those which are not in bucket  13+17+2+
        lengths = [len(task_prompt) + len(prefix) + len(suffix) + len(self.noun_bucket[noun]) for noun in noun_list] #关于任务和类别提示编码的长度

        # for noun in noun_list:
        #     temp = self.noun_bucket[noun]
        #     lengths_tmp = len(temp)

        embeddings = torch.stack([torch.cat([task_prompt, prefix, self.noun_bucket[noun], suffix]+ [self.pad_signal.expand(77 - length, -1)])
                for noun, length in zip(noun_list, lengths)])  # cls,77,512
    ...
    def _update_noun_features(self, noun_list, clip_model):
        ...
        if len(left_class_names) > 0:
            with torch.no_grad():
                tokens, name_lengths = clip.tokenize( left_class_names, return_length=True) #每一类包含多个单词，因此表示成句子[156， 77]，77为句子的最大长度
                name_lengths = [ n - 2 for n in name_lengths]   #只取真正的长度
                text_embeddings = clip_model.token_embedding(tokens.to(self.device)).type(clip_model.dtype) #转化成embeddings [156, 77, 512]
                text_embeddings = [ embedding[1 : 1 + length] for embedding, length in zip(text_embeddings, name_lengths)] #取有单词的位置对应的embeddings，并表示成list
            
            self.noun_bucket.update({name: embedding for name, embedding in zip(left_class_names, text_embeddings)})#self.noun_bucket 存放的是列别的文本表示
    ...
    def get_text_feature(x, indices, clip_model):
        ...
        if not torch.is_tensor(clip_model.text_projection):
            ...
            temp= x[torch.arange(x.shape[0]), indices]
            x = x[torch.arange(x.shape[0]), indices] @ clip_model.text_projection #只取最后一个维度认为包含了时序特征
        return x
