...
from .transforms import fliplr_joints, crop, generate_target, transform_pixel, show_preds
import random
...
class MaskFormer300WMapper:
    ...
    def __init__(
        ...
        *,
        ...
        self.flip = flip
        self.landmark_boundary_300W = landmark_boundary_300W
    ...
    def from_config(cls, cfg, is_train=True, modes=None):
        ...
        ret = {
            ...
            'landmark_boundary_300W': cfg.DATASETS.LANDMAKR_BOUNDARY_300W,
    ...
    def __call__(self, dataset_dict):
        ...
        img = np.array(Image.open(dataset_dict["file_name"]).convert('RGB'), dtype=np.float32)
        ...
        sto_maps=[]
        instances = Instances(self.input_size)  # 和image_shape一样大小的instances
        ...
        if self.is_train and self.modes=='prompt':
            ...
            boundary_index = self.landmark_boundary_300W[iidx]
            ...
            for idx in boundary_index:
                ...
                maps[map_index][int(y_low):int(y_high), int(x_low):int(x_high)] = 1 #mask 只统计一类目标，所以简单设置为1即可
