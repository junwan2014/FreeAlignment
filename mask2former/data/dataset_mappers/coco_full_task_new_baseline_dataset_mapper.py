...
class COCOFullTaskNewBaselineDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        if sem_seg_gt is not None:
            ...
            if len(sem_masks) == 0:
                # Some image does not have annotation (all ignored)
                sem_seg_instances.gt_masks = torch.zeros( (0, sem_seg_gt.shape[-2], sem_seg_gt.shape[-1]))
            else:
                sem_masks = BitMasks(torch.stack( [torch.from_numpy(np.ascontiguousarray(x.copy())) for x in sem_masks ]))
