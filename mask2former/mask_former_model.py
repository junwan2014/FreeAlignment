...
from lib.loss import SetCriterion_Wan
from .modeling.matcher import HungarianMatcher, HungarianMatcher_Wan
...
class MaskFormer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.criterion_land = criterion_land
    ...
    def from_config(cls, cfg):
        ...
        matcher_land = HungarianMatcher_Wan(cfg.MODEL.SEM_SEG_HEAD.LAND_CLASSES, cost_class=1.0, cost_coord=5.0)


        weight_dict = {"loss_ce": 1, "loss_mask": 1*mask_weight, "loss_dice": 1*dice_weight}
        print_dict = {"loss_ce": 1, "loss_mask": 1*mask_weight, "loss_dice": 1*dice_weight}
        if deep_supervision:
        ...
        weight_dict_land = {'loss_ce_land': 5, 'loss_land_coords': 1*cfg.MODEL.EXTRA.KPT_LOSS_COEF}
        print_dict.update(weight_dict_land)

        if cfg.MODEL.EXTRA.AUX_LOSS:
            aux_weight_dict = {}
            for i in range(cfg.MODEL.MASK_FORMER.DEC_LAYERS):
                aux_weight_dict.update(
                    {k + f'_{i}': v for k, v in weight_dict_land.items()})
            weight_dict_land.update(aux_weight_dict)

        criterion_land = SetCriterion_Wan(
            cfg.MODEL.SEM_SEG_HEAD.LAND_CLASSES,
            matcher_land,
            weight_dict_land,
            print_dict,
            cfg.MODEL.EXTRA.EOS_COEF,
            ['labels', 'kpts', 'cardinality']
        )
