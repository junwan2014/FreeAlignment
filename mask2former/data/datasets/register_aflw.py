...
from detectron2.data.datasets.coco import load_coco_json, load_sem_seg_aflw
...
def register_aflw(root, max_num):
    root = root+'AFLW/'
    meta = _get_voc_meta(CLASS_NAMES)
    image_dir = os.path.join(root, 'matlab_get_aflw/')
    gt_dir = os.path.join(root, 'matlab_get_aflw/')
    train_json = root + '/face_landmarks_aflw_train.csv'
    val_json = root + '/face_landmarks_aflw_test.csv'
    all_name = 'AFLW_train'
    DatasetCatalog.register(all_name, lambda: load_sem_seg_aflw(image_dir, train_json, max_num, 'train')) #D:\python_work\lib\detectron_repo\detectron2\data\datasets\coco.py
    ...
    all_name = 'AFLW_val'
    panoptic_root = 'E:/datasets/UFA/annotations/AFLW_val/'
    panoptic_json = 'E:/datasets/UFA/annotations/AFLW_gt_ibug.json'
    meta = _get_voc_meta(CLASS_NAMES_VAL)
    DatasetCatalog.register(all_name,lambda: load_sem_seg_aflw(image_dir, val_json, max_num, 'test'),)
