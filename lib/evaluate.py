...
from mask2former.data.transforms import transform_preds, transform_pixel_float, get_transform
...
def compute_nme(preds, targets, boxes=None):
    if isinstance(preds,torch.Tensor):
        preds = preds.numpy()
    target = targets.cpu().numpy()
    ...
    for i in range(N):
        ...
        if L == 19:  # aflw
            ...
            # interocular = np.linalg.norm(pts_gt[8, ] - pts_gt[9, ])
            interpupil = np.linalg.norm(pts_gt[16,] - pts_gt[17,])
            interocular = interpupil
        elif L == 68:  # 300w
            # interocular
            interocular = np.linalg.norm(pts_gt[36, ] - pts_gt[45, ])
            # lcenter = (
...
def get_transformer_coords(preds,meta,output_size):
    ...
    for i in range(b):
        center = meta[i]['center']
        scale = meta[i]['scale']
        rotate = meta[i]['rotate']
