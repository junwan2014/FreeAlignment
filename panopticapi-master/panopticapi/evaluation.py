...
def pq_compute_single_core(proc_id, annotation_set, gt_folder, pred_folder, categories):
    ...
    for gt_ann, pred_ann in annotation_set:
        ...
        pan_gt_tmp = np.array(Image.open(os.path.join(gt_folder, gt_ann['file_name'])), dtype=np.uint32)
        pan_gt = rgb2id(pan_gt_tmp)
        # plt.imshow(pan_gt)
        # plt.show()
        ...
        labels, labels_cnt = np.unique(pan_pred, return_counts=True) #每个query索引作为一类，标记号为query对应的id
        labels_tmp = labels
        for label, label_cnt in zip(labels, labels_cnt):#设置每个有效的query对应的area
        ...
        for label_tuple, intersection in gt_pred_map.items():
            ...
            if gt_segms[gt_label]['iscrowd'] == 1:
                continue
            if gt_segms[gt_label]['category_id'] != pred_segms[pred_label]['category_id']: #预测的和真实的seg的category_id不一致
                continue
