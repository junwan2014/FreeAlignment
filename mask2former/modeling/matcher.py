...
class HungarianMatcher_Wan(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, targets, landmark_index):
        ## target: [bs, 17, 2]
        bs, num_queries = outputs["pred_land_logits"].shape[:2]

        # We flatten to compute the cost matrices in a batch
        out_prob = outputs["pred_land_logits"].softmax(-1)  # [batch_size, num_queries, num_classes]
        out_kpt = outputs["pred_land_pts"]  # [batch_size, num_queries, 2]
...
class HungarianMatcher(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            # but approximate it in 1 - proba[target class].
            # The 1 is a constant that doesn't change the matching, it can be ommitted.
            cost_class = -out_prob[:, tgt_ids] #[100, 3]
            ...
            indices.append(linear_sum_assignment(C)) #任务指派函数，所有人的分配任务的效率最高
