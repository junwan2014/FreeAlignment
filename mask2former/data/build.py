...
def get_detection_dataset_dicts(
    ...
    for dataset_name, dicts in zip(names, dataset_dicts):
        dataset_dicts_temp = []
        dataset_dicts_temp.append(dicts)
        temp = list(itertools.chain.from_iterable(dataset_dicts_temp))
        dataset_dicts_tmp.append(temp)
    dataset_dicts = dataset_dicts_tmp
    ...
    if sampler is None:
        ...
        if sampler_name == "TrainingSampler":
            ...
            for ii in range(len(dataset)):
                sampler_tmp = TrainingSampler(len(dataset[ii]), shuffle=shuffle)
                sampler_list.append(sampler_tmp)
...
def build_detection_train_loader(
    ...
    *,
    ...
    if isinstance(dataset, list):
        dataset_tmp = []
        for ii in range(len(dataset)):
            dataset_temp = DatasetFromList(dataset[ii], copy=False)
            dataset_tmp.append(dataset_temp)
        dataset = dataset_tmp
    if mapper is not None:
        dataset_tmp = []
        for ii in range(len(dataset)):
            dataset_temp = MapDataset(dataset[ii], mapper[ii])
            dataset_tmp.append(dataset_temp)
        dataset = dataset_tmp
    if sampler is None:
    ...
    dataloder_list=[]
    for ii in range(len(dataset)): #D:\python_work\lib\detectron_repo\detectron2\data\build.py line 322
        temp = build_batch_data_loader(dataset[ii],sampler[ii],total_batch_size,aspect_ratio_grouping=aspect_ratio_grouping,num_workers=num_workers,)
        dataloder_list.append(temp)

    return dataloder_list
...
def build_detection_test_loader(
    ...
    # temp = build_batch_data_loader(dataset, batch_sampler, total_batch_size=1,
    #                                aspect_ratio_grouping=True, num_workers=num_workers, )
...
def dataset_sample_per_task_class_wan(cfg):
    dataset_dicts = get_detection_dataset_dicts(
        cfg.DATASETS.TRAIN,
        filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS,
        min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
